...
function msUntilClose(m: typeof MARKETS[number], now: Date): number {
  const t = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), m.closeH, m.closeM, 0))
  return t.getTime() - now.getTime()
}

function msUntilNextOpen(m: typeof MARKETS[number], now: Date): number {
  // Today's open (if still in future and it's a weekday)
  const todayOpen = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), m.openH, m.openM, 0))
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        <span style={{ fontFamily: 'var(--font-code)', fontWeight: 600, fontSize: 14, letterSpacing: '0.02em' }}>
          {fmtLocal(now)}
        </span>
        <span style={{ fontFamily: 'var(--font-code)', color: 'var(--color-text-muted)', fontSize: 11 }}>
          {fmtUtc(now)}
        </span>
      </div>
        ...
        return (
          <div key={m.name} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontWeight: 500, color: open ? 'var(--color-text)' : 'var(--color-text-muted)' }}>
                {m.name}
              </span>
              <span style={{
