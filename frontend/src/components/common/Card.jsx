      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <div className="flex items-baseline">
            ...
            {trend && trendValue && (
              <span className={`ml-2 body-small font-medium ${statusClasses[status]}`}>
      ...
      {(title || subtitle || actions) && (
        <div className="px-6 py-4 border-b border-gray-20">
          <div className="flex items-center justify-between">
            <div>
              {title && (
                <h3 className="heading text-gray-90">{title}</h3>
              )}
              {subtitle && (
                <p className="body-small text-gray-70 mt-1">{subtitle}</p>
              )}
    ...
    >
      <div className="p-4">
        <div className="flex items-start">
          ...
          <div className="flex-1">
            <h3 className={`heading font-semibold ${config.color}`}>
              {title}
            </h3>
            {message && (
    ...
    <Card variant="content" className={`text-center p-6 ${className}`}>
      {icon && (
        <div className="mx-auto mb-4 p-3 bg-primary-accent-light rounded-lg w-12 h-12 flex items-center justify-center">
          <div className="text-primary-teal text-xl">
            {icon}
          </div>
        </div>
      )}
