...
const StrategyTestTab = ({ strategyId }) => {
  ...
  const filteredTrades = selectedTest
    ? tradeFilter === "all"
      ? selectedTest.trades
      : selectedTest.trades.filter((tr) =>
          tradeFilter === "win" ? tr.pnl >= 0 : tr.pnl < 0,
        )
    : [];
      ...
      <div className="form-grid">
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) =>
              setTestParams((p) => ({ ...p, initial_capital: +e.target.value }))
            }
            min="1000"
            step="1000"
          />
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) =>
              setTestParams((p) => ({
                ...p,
                test_duration_days: +e.target.value,
              }))
            }
            min="1"
            max="365"
          ...
          >
            <div className="history-header">
              <span className="history-date">
                {new Date(ht.timestamp).toLocaleDateString()}
              </span>
              <span
                className={`history-profit ${ht.total_profit >= 0 ? "profit" : "loss"}`}
              >
                {formatCurrency(ht.total_profit)}
              </span>
            </div>
            <div className="history-details">
              <span>
                <span>Return</span>
                <span>{formatPercent(ht.return_pct)}</span>
              </span>
              <span>
                <span>Trades</span>
                <span>{ht.total_trades}</span>
              </span>
            </div>
          </InteractiveCard>
        ))}
    ...
    if (!test)
      return (
        <div className="no-test-selected">
          ...
          <h3>No Backtest Selected</h3>
          <p>Select a backtest from the history or run a new one.</p>
        </div>
      );
    ...
    return (
      <ContentCard title="Backtest Results" className="test-results">
        <div className="metrics-grid mb-8">
          <MetricCard
            title="Total P&L"
            value={formatCurrency(test.total_profit)}
            status={test.total_profit >= 0 ? "positive" : "negative"}
          />
          <MetricCard title="Return" value={formatPercent(test.return_pct)} />
          <MetricCard
            title="Sharpe Ratio"
            value={test.sharpe_ratio.toFixed(2)}
          />
          <MetricCard
            title="Max Drawdown"
            value={formatPercent(test.max_drawdown)}
          />
        ...
        <div className="content-grid content-grid-2 charts-section">
          <Card className="chart-container">
            ...
            <div className="chart-wrapper">
              <Line data={equityChart} options={equityChartOptions} />
            </div>
          </Card>
          <Card className="chart-container">
            <h4>Recent Trades</h4>
            <div className="chart-wrapper">
              <Bar data={tradesChart} options={chartOptions} />
            </div>
          </Card>
        </div>
        <div className="section-header">
          <h4 className="section-title">Trade History</h4>
        </div>
        <div className="trade-filter mb-2">
          <Button
            variant={tradeFilter === "all" ? "primary" : "ghost"}
            size="sm"
          ...
          <Button
            variant={tradeFilter === "win" ? "primary" : "ghost"}
            size="sm"
          ...
          <Button
            variant={tradeFilter === "loss" ? "primary" : "ghost"}
            size="sm"
