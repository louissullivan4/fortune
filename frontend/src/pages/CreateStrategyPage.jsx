...
const CreateStrategyPage = () => {
  ...
  const validateForm = () => {
    const errs = {};
    if (!formData.name || !formData.name.trim()) errs.name = 'Strategy name is required';
    ...
    for (const field of fields) {
      ...
      if (field.required && (val === undefined || (typeof val === 'string' && !val.trim()))) {
        errs[field.key] = `${field.label} is required`;
      }
      if (field.type === 'number' && field.min !== undefined && val < field.min) {
        errs[field.key] = `${field.label} must be ≥ ${field.min}`;
      }
      if (field.type === 'number' && field.max !== undefined && val > field.max) {
    ...
    if (algo === 'PairTrading') {
      ...
      if (formData.config.risk_per_trade !== undefined && (formData.config.risk_per_trade < 1 || formData.config.risk_per_trade > 100))
        errs.risk_per_trade = 'Must be between 1 and 100';
    } else if (algo === 'BollingerReversionStrategy') {
      if (formData.config.num_std !== undefined && formData.config.num_std < 0.1)
        errs.num_std = 'Num Std Dev must be ≥ 0.1';
      if (formData.config.risk_per_trade !== undefined && formData.config.risk_per_trade < 1)
              ...
              {ALGO_FIELDS[formData.config.algorithm].map(field => (
                FIELD_TIPS[field.key] && (
                  <li key={field.key} className="tip-item">
                    <span className="tip-label">{field.label}:</span> {FIELD_TIPS[field.key]}
                  </li>
                )
              ))}
            </ul>
