...
function msToSec(ms: number) { return Math.round(ms / 1000) }
...
export default function ConfigPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setCfg(updated); setDraft(updated); setSaved(true)
  ...
  if (!draft) return <div style={{ color: 'var(--color-text-muted)', fontSize: 13 }}>Loading...</div>
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button className="btn btn-secondary" onClick={() => setDraft(cfg!)} disabled={!isChanged || saving}>Reset</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div className="card">
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 13, color: 'var(--color-text-secondary)', display: 'block', marginBottom: 6 }}>
              Max position size — {(draft.maxPositionPct * 100).toFixed(0)}% of budget (€{(draft.maxBudgetEur * draft.maxPositionPct).toFixed(0)})
            </label>
            <input
              type="range"
        ...
        {/* Engine parameters */}
        <div className="card">
          <div className="section-label" style={{ marginBottom: 16 }}>engine parameters</div>

          <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <label style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>Cycle interval</label>
            ...
            {intervalUnit === 'seconds' ? (
              <>
                ...
                />
                <div style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 4 }}>Minimum 10 seconds</div>
              </>
            ) : (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 8 }}>
          ...
          <div style={{ padding: '12px 0', borderTop: '0.5px solid var(--color-border)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              <div className="section-label" style={{ marginBottom: 4 }}>mode</div>
              <div style={{ fontSize: 13, fontFamily: 'var(--font-code)', color: draft.trading212Mode === 'live' ? '#dc2626' : '#16a34a' }}>
      ...
      <div className="card" style={{ marginTop: 16 }}>
        <div className="section-label" style={{ marginBottom: 12 }}>trade universe ({draft.tradeUniverse.length} tickers)</div>
              ...
              {searchResults.map((inst) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '8px 12px', cursor: 'pointer', borderBottom: '0.5px solid var(--color-border)',
                    fontSize: 13,
                  }}
